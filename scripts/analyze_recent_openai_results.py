...
def process_results_file(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            if '"Score:' in line and '"verified_by_openai": true' in line:
                try:
...
def plot_accuracy_metrics(scores, save_dir='reports/recent_openai_accuracy'):
    ...
    plt.title('Distribution of Recent OpenAI Detection Scores')
    plt.xlabel('Score')
    plt.ylabel('Count')
    plt.legend()
...
def main():
    ...
    if not all_scores:
        print("No OpenAI-verified samples found in the recent results")
        return
    ...
    # Print summary
    print("\nRecent OpenAI Accuracy Analysis:")
    print(f"Total OpenAI-verified samples: {analysis['total_samples']}")
    ...
    print("\nVisualizations saved to reports/recent_openai_accuracy/")
